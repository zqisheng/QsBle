...
public class OnDescriptorWriteMessage extends AbsBleMessage implements ICallbackMessage {
    ...
    public final void onHandlerMessage() {
        ...
        if (BleDebugConfig.isOpenGattCallbackLog){
            BleLog.d(String.format("BleCallback OnDescriptorWriteMessage:mac=%s,status=%d,chac=%s,desc=%s,value=%s",device.getAddress(),status,descriptor.getCharacteristic().getUuid().toString(),descriptor.getUuid().toString(),  Utils.bytesToHexStr(value)));
        }
        GlobalBleCallback globalBleCallback = getSimpleBle().getGlobalBleGattCallback();
        if (globalBleCallback!=null){
            globalBleCallback.onDescriptorWrite(device,descriptor,value,status);
        }
